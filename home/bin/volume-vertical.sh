...
print_status() {
    ...
    # Отображение: 0 -> off; 100+ -> max; иначе двузначный процент
    if [ "$vol" -le 0 ]; then
        local vol_display="off"
    elif [ "$vol" -ge 100 ]; then
        local vol_display="max"
    else
        local vol_display=$(printf "%02d" $vol)
    fi
