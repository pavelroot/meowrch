...
brightness_icon() {
    ...
    local percent=$(echo $value | sed 's/%//')
    
    # Проверяем, если 0% -> off; 100% или больше -> max; иначе двузначный процент
    if [ "$percent" -le 0 ]; then
        local percent_display="off"
    elif [ "$percent" -ge 100 ]; then
        local percent_display="max"
    else
        local percent_display=$(printf "%02d" $percent)
    fi
